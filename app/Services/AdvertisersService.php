...
class AdvertisersService
{
    ...
    public function list(PaginateRequest $request)
    {
        try {
            ...
            return User::with('media', 'addresses')->role(EnumRole::ADVERTISER)->where(
    ...
    public function store(DeliveryBoyRequest $request)
    {
        try {
            DB::transaction(function () use ($request) {
                ...
                $this->user->assignRole(EnumRole::ADVERTISER);
            });
            return $this->user;
        } catch (Exception $exception) {
            DB::rollBack();
    ...
    {
        try {
            if (!in_array(EnumRole::ADVERTISER, $this->blockRoles)) {
    ...
    public function destroy(User $deliveryBoy)
    {
        try {
            if (!in_array(EnumRole::ADVERTISER, $this->blockRoles)) {
                if ($deliveryBoy->hasRole(EnumRole::ADVERTISER)) {
